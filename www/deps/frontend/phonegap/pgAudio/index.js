...
var pgMedia = window.Media;
var pgAudio = window.Audio;
...
var pgLocalStorage = require('pgStorage').localStorage;
...
AudioFile.prototype.load = function (encodedUrl, cb) {
	...
	if (lowLatencyAudio) {
		...
		this.media = new pgMedia(window.decodeURI(encodedUrl), loadSuccess, loadFailure);
	} else if (pgAudio) {
		this.audio = new pgAudio(encodedUrl);
...
function BgmChannel(name, options) {
	...
	if (pgLocalStorage.get('audio.' + this.name + '.mute')) {
		this._isMuted = true;
	}
}
...
BgmChannel.prototype.mute = function () {
	if (!this._isMuted) {
		...
		pgLocalStorage.set('audio.' + this.name + '.mute', true);
	}
};
...
BgmChannel.prototype.unmute = function () {
	if (this._isMuted) {
		...
		pgLocalStorage.del('audio.' + this.name + '.mute');
	}
};
