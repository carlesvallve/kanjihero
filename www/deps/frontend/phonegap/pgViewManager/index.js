...
wrapper.init = function (deviceDimensions) {
	plugin = window.wizViewManager || null;

	ua = window.navigator.userAgent.toLowerCase();
	isMobileDevice = ua.match(/ipad/) || ua.match(/ipod/) || ua.match(/iphone/) || ua.match(/webos/) || ua.match(/android/);

	deviceDimensions = deviceDimensions || defaultDeviceDimension;
	...
	wrapper.deviceDimensions = deviceDimensions;
...
// Increment on every new wiz view creation
var viewZIndexIncrementCount = 0;
...
function postPayload(viewName, payload) {
	...
	if (isWizView) {
		...
		// phonegap plugin not found, assume iframe
...
View.prototype.create = function (options, cb) {
	...
	if (plugin) {
		...
		this.iframe.style.zIndex = (options.isPopup ? 100 : 0) + 900 + viewZIndexIncrementCount;
