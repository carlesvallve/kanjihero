/**
 * @class
 * @classDesc Astar pathfinding algorithm
 
 * @param Grid bidimensional array with 0=walkable 1=unwalkable values
 * @param Start starting point in array format
 * @param Goal ending point in array format
 * @param Find string defining the searchpath mode
...
function AStar(Grid, Start, Goal, Find) {
	...
	var abs = Math.abs,
		...
		rows = Grid.length,
		limit = cols * rows,
		Distance = { Diagonal: Diagonal, DiagonalFree: Diagonal, Euclidean: Euclidean, EuclideanFree: Euclidean, Manhattan: Manhattan }[Find] || Manhattan;

	return Path(AStarFunk());
